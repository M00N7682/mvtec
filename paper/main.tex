...
표~\ref{tab:main}에서 베이스라인 합성은 특히 $K=1$에서 No synth 대비 평균 AUROC를 개선하는 경향을 보이며, $K$가 증가함에 따라 추가 합성의 이득은 감소한다.
한편 정상-only 계열인 PaDiM-lite는 $K$ 변화에 크게 민감하지 않고 안정적으로 높은 AUROC를 보인다.
이 관찰은 ``소수 결함 라벨을 활용한 지도 학습''과 ``정상-only 이상 탐지''가 서로 다른 장단점을 가진다는 점을 시사한다.
